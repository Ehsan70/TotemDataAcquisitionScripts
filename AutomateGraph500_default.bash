...
do
        ...
        echo "[BASH] Running the GPU only graph500 for scale $SCALE" > ./out/HYB_s$SCALE.txt

        ../graph500 -s$SCALE -o /local/tmp/edgelist$SCALE -t "-p2 -c" >> ./out/HYB_s$SCALE.txt
...
do
        ...
        echo "[BASH] Running the GPU only graph500 for scale $SCALE" > ./out/GPU_s$SCALE.txt

        ../graph500 -o /local/tmp/edgelist$SCALE -s$SCALE -t "-p1" >> ./out/GPU_s$SCALE.txt
...
do
	...
	echo "[BASH] Running the CPU only (default) graph500 for scale $SCALE" > ./out/CPU_s$SCALE.txt

	../graph500 -o /local/tmp/edgelist$SCALE -s$SCALE >> ./out/CPU_s$SCALE.txt
